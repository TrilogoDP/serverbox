<?php
class ControllerExtensionModuleLuxshopArticleFeatured extends Controller {
	public function index($setting) {
		...
		if (!empty($setting['article'])) {
			...
			foreach ($articles as $article_id) {
				$article_info = $this->model_luxshop_article->getArticle($article_id);
				...
				if ($article_info) {
					...
					$data['articles'][] = array(
						...
						'href' => $this->url->link('luxshop/article', '&ls_news_article_id=' . $article_info['article_id'])
					);
				}
			}
		}
		...
		$data['module'] = $module++;
		if ($data['articles']) {
			return $this->load->view('extension/module/luxshop_article_featured', $data);
		}
		}
	}
}