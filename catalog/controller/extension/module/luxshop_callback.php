<?php
class ControllerExtensionModuleLuxshopCallback extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->load->model('extension/module/luxshop_callback');
		$this->load->language('extension/module/luxshop_callback');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['action'])) {
			if ($this->validate()) {
				...
				$results = $this->model_extension_module_luxshop_callback->addCallback($data);
			...
			$this->response->setOutput($this->load->view('extension/module/luxshop_callback', $data));
		} else {
	...
	private function sendMail($data) {
		$this->language->load('extension/module/luxshop_callback');
		$text = '';
		$callbackpro = $this->config->get('callbackpro');
