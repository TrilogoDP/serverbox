<?php
class ControllerExtensionModuleLuxshopNewfastorder extends Controller {
	private $error = array();
	public function index() {
		...
		$this->language->load('extension/module/luxshop_newfastorder');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['action'])) {
			...
			if ($this->validate()) {
				...
				$this->load->model('extension/module/luxshop_newfastorder');
				...
				$results = $this->model_extension_module_luxshop_newfastorder->addOrder($data, $product_options);
			...
			$this->response->setOutput($this->load->view('extension/module/luxshop_newfastorder', $data));
	...
	{
			$res = $this->_read_url('http://my.smscab.ru/sys/send.php?login='.urlencode($login).'&psw='. $password .
