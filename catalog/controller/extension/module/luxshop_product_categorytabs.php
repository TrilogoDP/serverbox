<?php
class ControllerExtensionModuleLuxshopProductCategorytabs extends Controller {
	public function index($setting) {
		static $module = 0;
		$this->document->addScript('catalog/view/theme/luxshop/js/dragscroll.js');
		$this->load->model('extension/module/luxshop_productcategorytabs');
		$data['ajaxurl'] = 'index.php?route=extension/module/luxshop_product_categorytabs/ajaxLoadProduct';
		...
		if(!empty($categories)){
			$category_info = $this->model_extension_module_luxshop_productcategorytabs->getCategories($categories);
		}

		if (!empty($category_info)) {
			foreach ($category_info as $category) {
				if($filter_sub_category == 1){
					$cp2c = $this->model_extension_module_luxshop_productcategorytabs->checkSubCategories((int)$category['category_id']);
				} else {
					$cp2c = $this->model_extension_module_luxshop_productcategorytabs->checkCategory((int)$category['category_id']);
				}
			...
			$data['module'] = $module++;
			return $this->load->view('extension/module/luxshop_product_categorytabs', $data);
		}
	}
	...
	public function ajaxLoadProduct() {
		if (isset($this->request->post['module_id']) && (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')) {
			...
			$results = $this->model_extension_module_luxshop_productcategorytabs->getProductToCategoryTabSelect($filter_data);
			...
			if ($results) {
				...
				$this->response->setOutput($this->load->view('extension/module/luxshop_product_list', $data));
			}
		}
	}
}
