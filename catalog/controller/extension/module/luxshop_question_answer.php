<?php
class ControllerExtensionModuleLuxshopQuestionAnswer extends Controller {
	...
	public function getList() {
		...
		$question_answer_total = $this->model_extension_module_luxshop_question_answer->getTotalQuestionAnswer($this->request->get['product_id']);

		$results = $this->model_extension_module_luxshop_question_answer->getQuestionAnswer($this->request->get['product_id'], ($page - 1) * 5, 5);
		...
		$pagination->limit = 5;
		$pagination->url = $this->url->link('extension/module/luxshop_question_answer/getList', 'product_id=' . $data['product_id'] . '&page={page}');
		...
		$this->response->setOutput($this->load->view('extension/module/luxshop_question_answer', $data));
	...
	public function validateForm() {
		if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			$json = array();
			$this->language->load('extension/module/luxshop_question_answer');
			$this->load->model('extension/module/luxshop_question_answer');
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				$qadata = (array)$this->config->get('qadata');

				if (isset($qadata['name_field']) && (isset($qadata['name_field_required']))) {
					if ((utf8_strlen(trim($this->request->post['name_field'])) < 1) || (utf8_strlen(trim($this->request->post['name_field'])) > 32)) {
						$json['error']['name_field'] = $this->language->get('error_name_field');
					}
				}

				if (isset($qadata['telephone_field']) && (isset($qadata['telephone_field_required']))) {
					if ((utf8_strlen($this->request->post['telephone_field']) < 3) || (utf8_strlen($this->request->post['telephone_field']) > 20)) {
				...
				if (isset($qadata['email_field']) && (isset($qadata['email_field_required']))) {
					if (((utf8_strlen($this->request->post['email_field']) > 96) || !preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $this->request->post['email_field']))) {
				...
				if (!isset($json['error'])) {
					...
					$this->model_extension_module_luxshop_question_answer->SaveQuestionAnswer($data_save_post);
