<?php
class ControllerExtensionModuleLuxshopReviewscustomer extends Controller {
    ...
    public function index($setting) {
        ...
        $this->language->load('extension/module/luxshop_reviewscustomer');
        ...
        $this->load->model('catalog/luxshop_reviewscustomer');
        ...
        $reviews_total = $this->model_catalog_luxshop_reviewscustomer->getTotalReviews($filter_data);
        if ($reviews_total > $limit_max) {
            $reviews_total = $limit_max;
        }
        if ($setting['order_type'] == 'last') {
            $results = $this->model_catalog_luxshop_reviewscustomer->getLatestCustomerReviews($filter_data);
        } else {
            $results = $this->model_catalog_luxshop_reviewscustomer->getRandomCustomerReviews($filter_data);
        }

        foreach ($results as $result) {
            if ($result['image']) {
                $thumb = $this->model_tool_image->resize($result['image'], 95, 95);
            } else {
                $thumb = $this->model_tool_image->resize('placeholder.png', 95, 95);
            }
            ...
            $data['reviews'][] = array(
                ...
                'description' => utf8_substr(strip_tags(html_entity_decode($result['text'], ENT_QUOTES, 'UTF-8')), 0, 220),
        ...
        $data['link_all_reviews'] = $this->url->link('product/luxshop_reviewscustomer');
        ...
        if($data['reviews']){
            return $this->load->view('extension/module/luxshop_reviewscustomer', $data);
        }
    }
}