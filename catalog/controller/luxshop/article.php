<?php
class ControllerLuxshopArticle extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->load->language('luxshop/article');
		...
		if (isset($this->request->get['ls_news_id'])) {
			...
			$ls_news_id = (int)array_pop($parts);

			foreach ($parts as $news_id) {
				if (!$ls_news_id) {
					$ls_news_id = $news_id;
				} else {
					$ls_news_id .= '_' . $news_id;
				}

				$category_info = $this->model_luxshop_category->getCategory($news_id);

				if ($category_info) {
					$data['breadcrumbs'][] = array(
						'text' => $category_info['name'],
						'href' => $this->url->link('luxshop/category', 'ls_news_id=' . $ls_news_id)
					);
				}
			}

			$category_info = $this->model_luxshop_category->getCategory($ls_news_id);

			if ($category_info) {

				$data['breadcrumbs'][] = array(
					'text' => $category_info['name'],
					'href' => $this->url->link('luxshop/category', 'ls_news_id=' . $this->request->get['ls_news_id'])
				);
			}
		}

		if (isset($this->request->get['ls_news_article_id'])) {
			$article_id = (int)$this->request->get['ls_news_article_id'];
		} else {
			$article_id = 0;
		}


		$this->load->model('luxshop/article');

		$info_article = $this->model_luxshop_article->getArticle($article_id);

		if ($info_article) {

			$data['breadcrumbs'][] = array(
				'text' => $info_article['name'],
				'href' => $this->url->link('luxshop/article', '&ls_news_article_id=' . $this->request->get['ls_news_article_id'])
			);
			...
			$this->document->addLink($this->url->link('luxshop/article', 'ls_news_article_id=' . $this->request->get['ls_news_article_id']), 'canonical');
			...
			$data['article_id'] = (int)$this->request->get['ls_news_article_id'];
			...
			$ls_article_width = (!empty($this->config->get('ls_article_width')) ? $this->config->get('ls_article_width') : 400);
			$ls_article_height = (!empty($this->config->get('ls_article_height')) ? $this->config->get('ls_article_height') : 400);
			...
			if ($info_article['image']) {
				$data['thumb'] = $this->model_tool_image->resize($info_article['image'], $ls_article_width, $ls_article_height);
			} else {
				$data['thumb'] = '';
			}
			...
			$ls_article_fp_width = (!empty($this->config->get('ls_article_fp_width')) ? $this->config->get('ls_article_fp_width') : 200);
			$ls_article_fp_height = (!empty($this->config->get('ls_article_fp_height')) ? $this->config->get('ls_article_fp_height') : 200);


			$results = $this->model_luxshop_article->getProductFeatured($this->request->get['ls_news_article_id']);

			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], $ls_article_fp_width, $ls_article_fp_height);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', $ls_article_fp_width, $ls_article_fp_height);
				}
				...
				if(isset($data['setting_module']['status_additional_image_hover']) && ($data['setting_module']['status_additional_image_hover'] == 1)){
					...
					foreach ($results_img as $key => $result_img) {
						if ($key < 1) {
							$additional_image_hover = $this->model_tool_image->resize($result_img['image'], $ls_article_fp_width, $ls_article_fp_height);
						}
					}
				}
				if ((float)$result['special']) {
				...
				$data['products'][] = array(
					'width' 				=> $ls_article_fp_width,
					'height' 				=> $ls_article_fp_height,
			...
			$this->model_luxshop_article->updateViewedArticle($this->request->get['ls_news_article_id']);
			...
			$this->response->setOutput($this->load->view('luxshop/article', $data));
		} else {

			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_error'),
				'href' => $this->url->link('luxshop/article', '&ls_news_article_id=' . $ls_news_article_id)
			);
	...
	public function article_review() {
		...
		$review_total = $this->model_luxshop_article->getTotalReviewsByArticleId($this->request->get['ls_news_article_id']);

		$results = $this->model_luxshop_article->getReviewsByArticleId($this->request->get['ls_news_article_id'], ($page - 1) * 5, 5);
		...
		$pagination->limit = 5;
		$pagination->url = $this->url->link('luxshop/article/article_review', 'ls_news_article_id=' . $this->request->get['ls_news_article_id'] . '&page={page}');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($review_total) ? (($page - 1) * 5) + 1 : 0, ((($page - 1) * 5) > ($review_total - 5)) ? $review_total : ((($page - 1) * 5) + 5), $review_total, ceil($review_total / 5));

		$this->response->setOutput($this->load->view('luxshop/article_review', $data));
	}
	public function write_review() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				...
				$this->model_luxshop_article->addArticleReview($this->request->get['article_id'], $this->request->post);
	...
	public function likeDislike() {
		if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			...
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				$this->load->model('luxshop/article');
				$article_review_id = $this->model_luxshop_article->addLikeDislike($this->request->post);
				$json = $this->model_luxshop_article->getLikeDislike($article_review_id);
			}
