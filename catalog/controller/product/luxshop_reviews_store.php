<?php
class ControllerProductLuxshopReviewsStore extends Controller {
	public function index() {
		$this->document->addStyle('catalog/view/theme/luxshop/stylesheet/popup-reviews-store/stylers.css');
		$this->load->language('product/luxshop_reviews_store');
		$this->load->model('catalog/luxshop_reviews_store');
		$data['reviews_theme_rating'] = $this->model_catalog_luxshop_reviews_store->getReviewsThemeRating();
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('product/luxshop_reviews_store', $url)
		);
		...
		$data['rating_store'] = $this->model_catalog_luxshop_reviews_store->getSumAvgReviewsStore();
		$data['total_rs'] = $this->model_catalog_luxshop_reviews_store->getTotalReviewsStore();
		$data['percent_rating'] = $this->model_catalog_luxshop_reviews_store->getPercentReviewsStore();
		...
		$reviews_total = $this->model_catalog_luxshop_reviews_store->getTotalReviewsStore();
		$results = $this->model_catalog_luxshop_reviews_store->getAllReviews($filter_data);
		foreach ($results as $result) {
			$data['reviews_store'][] = array(
				...
				'avg_customer_rating'   => $this->model_catalog_luxshop_reviews_store->getAvgRatingCustomer($result['reviews_store_id']),
		...
		$data['sorts'][] = array(
			...
			'href'  => $this->url->link('product/luxshop_reviews_store', 'sort=date_added_desc' . $url)
		);
		$data['sorts'][] = array(
			'text'  => $this->language->get('text_date_added_asc'),
			'value' => 'date_added_asc',
			'href'  => $this->url->link('product/luxshop_reviews_store', 'sort=date_added_asc' . $url)
		);
		...
		$pagination->url = $this->url->link('product/luxshop_reviews_store', $url . '&page={page}');
		...
		if ($page == 1) {
		    $this->document->addLink($this->url->link('product/luxshop_reviews_store', '', 'SSL'), 'canonical');
		} elseif ($page == 2) {
		    $this->document->addLink($this->url->link('product/luxshop_reviews_store', '', 'SSL'), 'prev');
		} else {
		    $this->document->addLink($this->url->link('product/luxshop_reviews_store', 'page='. ($page - 1), 'SSL'), 'prev');
		}

		if ($limit && ceil($reviews_total / $limit) > $page) {
		    $this->document->addLink($this->url->link('product/luxshop_reviews_store', 'page='. ($page + 1), 'SSL'), 'next');
		}
		...
		$this->response->setOutput($this->load->view('product/luxshop_reviews_store', $data));
		} else {
			$url = '';
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$all_review_theme = $this->model_catalog_luxshop_reviews_store->getAllReviewTheme();
			...
			if (!isset($json['error'])) {
				$this->model_catalog_luxshop_reviews_store->addReviewStore($this->request->post);
	...
	public function popupFormReviewStore() {
		...
		$data['reviews_theme'] = $this->model_catalog_luxshop_reviews_store->getReviewsTheme();
		$data['reviews_store_setting'] = $this->config->get('reviews_store_setting');
		...
		$this->response->setOutput($this->load->view('product/luxshop_reviews_store_form', $data));
	}
	...
	public function likeDislike() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			$this->load->model('catalog/luxshop_reviews_store');
			$reviews_store_id = $this->model_catalog_luxshop_reviews_store->addLikeDislike($this->request->post);
			$json = $this->model_catalog_luxshop_reviews_store->getLikeDislike($reviews_store_id);
		}
