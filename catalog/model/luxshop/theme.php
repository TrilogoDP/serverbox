<?php
class ModelLuxshopTheme extends Model {
	...
	public function getLatest($data = array()) {
		$product_data = $this->cache->get('product.ns_latest_grid.' . (int)$data['filter_category_id'] . '.' . (int)$data['filter_sub_category'] . '.'. (int)$data['limit_max'] . '.' . (int)$data['start'] . '.'. (int)$data['limit'] . '.'. (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id'));
		if (!$product_data) {
		$sql = "SELECT p.product_id, p.image, p.name, p.quantity, p.price, p.date_available FROM (SELECT p.product_id, p.image, pd.name, p.quantity, p.price, p.date_available, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
		...
		$sql .= " ORDER BY p.date_available DESC, (p.price>0) DESC,(p.image>'') DESC,(p.quantity>0) DESC, LCASE(pd.name) DESC";
		$sql .= " LIMIT  0," . (int)$data['limit_max'];
		$sql .= ") p ORDER BY p.date_available DESC,(p.price>0) DESC,(p.image>'') DESC,(p.quantity>0) DESC, LCASE(p.name) DESC";
		...
		foreach ($query->rows as $result) {
			...
			$this->cache->set('product.ns_latest_grid.' . (int)$data['filter_category_id'] . '.' . (int)$data['filter_sub_category'] . '.'. (int)$data['limit_max'] . '.' . (int)$data['start'] . '.'. (int)$data['limit'] . '.'. (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id'), $product_data);
		}
		return $product_data;
	}
}
