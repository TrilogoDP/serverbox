...
define('LUXSHOP_VERSION', '1.0');
class Luxshop_Minifier {
    private $path;
    private $dir = 'ls-cache/';
    ...
    private function getHash($files, $ext) {
        ...
        $hash .= LUXSHOP_VERSION;
        $hash .= $this->getHostName();
        return md5($hash) . '.' . $ext;
    }

    private function printStyle($href) {
        return '<link rel="stylesheet" href="' . $this->staticAsset($this->addLuxShopVersion($href, $this->minify_css)) . '"/>' . PHP_EOL;
    }

    private function printScript($src, $def = '') {
        $def = rtrim($def) . ' ';
        return '<script' . $def . 'src="' . $this->staticAsset($this->addLuxShopVersion($src, $this->minify_css)) .  '"></script>' . PHP_EOL;
    }
    ...
    private function addLuxShopVersion($url, $is_minified) {
        if (!$this->getDeveloperMode() && $is_minified) {
            return $url . '?ls2v=' . LUXSHOP_VERSION;
        }
        if (strpos($url, '?') === false) {
            return $url . '?ls2v=' . LUXSHOP_VERSION;
        }
