...
class ControllerSaleOrder extends Controller {
	private $error = array();
				...
				public function setOrderStatus() {
					...
					if (!$json) {									
						...
						$this->load->model('extension/module/luxshop_ordersetting');		
						$status_info = $this->model_extension_module_luxshop_ordersetting->getStatusSendOnOff($order_status_id);
				...
				private function sendMailOrderStatus($order_id, $order_status_id) {
					...
					$order_info = $this->model_extension_module_luxshop_ordersetting->getOrderStatusSendMail($order_id);
					...
					$subject  = $this->model_extension_module_luxshop_ordersetting->getCustomFields($order_info, $subject_info);
					}	
					...
					$html_message  = $this->model_extension_module_luxshop_ordersetting->getCustomFields($order_info, $html_message_info);
					}
				...
				private function sendMail($number_ttn, $email_user, $order_id) {
					...
					$subject = $this->model_extension_module_luxshop_ordersetting->getCustomFields($order_info, $subject_text[$order_info['language_id']]['text']);
					$html_message  = $this->model_extension_module_luxshop_ordersetting->getCustomFields($order_info, $html_message_text[$order_info['language_id']]['text']);
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			'calculated_summ'     		=> $this->currency->format($calculated_summ2, $result['currency_code'], '', true, $result['currency_code'], $result['currency_value']),
