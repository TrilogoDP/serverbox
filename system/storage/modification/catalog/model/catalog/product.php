	...
	public function updateViewed($product_id) {
			...
			if ($product_id && ($product_id !=0)) {
				...
				if (isset($this->request->cookie['productviewed'])) {
					$productviewed = array_merge($productviewed, explode(',', $this->request->cookie['productviewed']));
				}

				$productviewed = array_diff($productviewed, [(int)$product_id]);

				array_unshift($productviewed, (int)$product_id);

				setcookie('productviewed', implode(',',$productviewed), time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
			}
