...
class SeoPro {
    ...
    public function prepareRoute($parts) {
        ...
        if (!empty($parts) && is_array($parts)) {
            ...
            foreach($parts as $id => $part) {
				...
				if (!isset($this->request->get['ls_news_id'])) {
					$this->request->get['ls_news_id'] = $url[1];
				} else {
					$this->request->get['ls_news_id'] .= '_' . $url[1];
				}
        ...
        if (isset($this->request->get['product_id'])) {
			...
			} elseif (isset($this->request->get['ls_news_article_id'])) {
				$this->request->get['route'] = 'luxshop/article';
				if (!isset($this->request->get['ls_news_id'])) {
					$path = $this->getPathByLsNewsArticle($this->request->get['ls_news_article_id']);
					if ($path) $this->request->get['ls_news_id'] = $path;
				}
			} elseif (isset($this->request->get['ls_news_id'])) {
				$this->request->get['route'] = 'luxshop/category';
    ...
    public function baseRewrite($data, $language_id) {
        ...
        switch ($data['route']) {
			...
			case 'luxshop/article':
				if (isset($data['ls_news_article_id'])) {
					$tmp = $data;
					$data = array();
					$data['ls_news_id'] = $this->getPathByLsNewsArticle($tmp['ls_news_article_id']);
					if (!$data['ls_news_id']) return $link;
					$data['ls_news_article_id'] = $tmp['ls_news_article_id'];
				}
				break;
			case 'luxshop/category':
				if (isset($data['ls_news_id'])) {
					$ls_category = explode('_', $data['ls_news_id']);
					$ls_category = end($ls_category);
					$data['ls_news_id'] = $this->getPathByLsNewsCategory($ls_category);
					if (!$data['ls_news_id']) return $link;
				}
				break;
			...
			case 'luxshop/article/article_review':
				return [$url, $data, $postfix];
				break;
			...
			case 'extension/module/luxshop_question_answer/getList':
                return [$url, $data, $postfix];
                break;
        ...
        foreach ($data as $key => $value) {
            ...
            switch ($key) {
				...
				case 'ls_news_id':
				$ls_categories = explode('_', $value);
					foreach($ls_categories as $ls_category) {
						$queries[] = 'ls_news_id=' . $ls_category;
					}
					unset($data[$key]);
					break;
				case 'ls_news_article_id':
					$article_id = (int)$value;
					$queries[] = 'ls_news_article_id=' . $article_id;
					$postfix = true;
					unset($data[$key]);
					break;	
	...
	private function getPathByLsNewsArticle($article_id) {
		$article_id = (int)$article_id;
		if ($article_id < 1) return false;

		static $path = null;
		if (!isset($path)) {
			$path = $this->cache->get('ls_news_article.seopath');
			if (!isset($path)) $path = array();
		}

		if (!isset($path[$article_id])) {
			$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "ls_news_articles_to_category WHERE article_id = '" . $article_id . "' ORDER BY main_category DESC LIMIT 1");

			$path[$article_id] = $this->getPathByLsNewsCategory($query->num_rows ? (int)$query->row['category_id'] : 0);

			$this->cache->set('ls_news_article.seopath', $path);
		}

		return $path[$article_id];
	}
	private function getPathByLsNewsCategory($category_id) {
		$category_id = (int)$category_id;
		if ($category_id < 1) return false;

		static $path = null;
		if (!isset($path)) {
			$path = $this->cache->get('ls_news_category.seopath');
			if (!isset($path)) $path = array();
		}
		...
		if (!isset($path[$category_id])) {
			...
			$sql .= ") AS path FROM " . DB_PREFIX . "ls_news t0";
			for ($i = 1; $i < $max_level; ++$i) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "ls_news t$i ON (t$i.category_id = t" . ($i-1) . ".parent_id)";
			}
			...
			$this->cache->set('ls_news_category.seopath', $path);
		}
