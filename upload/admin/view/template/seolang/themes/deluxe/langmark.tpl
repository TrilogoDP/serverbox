					...
					<?php foreach ($languages as $language) { ?>
						<li>
							<?php if ($language['main']) { ?>
								<button class="language-call btn btn-link btn-block language-select" type="button" onclick="lm_deleteCookie('languageauto'); window.location = '<?php echo $language['url']; ?>'">
								<?php } else { ?>
									<button class="language-call btn btn-link btn-block language-select" type="button" onclick="lm_setCookie('languageauto', '1', {expires: 180}); window.location = '<?php echo $language['url']; ?>'">
									<?php } ?>
