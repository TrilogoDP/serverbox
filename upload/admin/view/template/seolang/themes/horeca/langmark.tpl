<?php if (count($languages) > 1) { ?>
	...
	<div class="pull-left lang-horeca">
				...
				<button class="btn btn-link dropdown-toggle" data-toggle="dropdown">
					<?php foreach ($languages as $language) { ?>
						<?php if ($language['current']) { ?>
							...
							<?php } ?>
							<span class="hidden-xs hidden-sm hidden-md"><?php echo $language['name']; ?></span></button>
			<?php } ?>
		<?php } ?>
				...
				<li>
					<?php if ($language['main']) { ?>
						...
						<?php } else { ?>
							...
							<?php if (isset($settings_widget['image_status']) && $settings_widget['image_status']) { ?>
								<img class="langChoose" src="catalog/language/<?php echo $language['code']; ?>/<?php echo $language['code']; ?>.png" alt="<?php echo $language['name']; ?>" title="<?php echo $language['name']; ?>" />
							<?php } ?>

							<span></span><?php echo $language['name']; ?></button>
				</li>
			<?php } ?>
		</ul>
			</div>
