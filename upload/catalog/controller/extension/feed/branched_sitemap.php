 ...
 * @copyright  © Serge Tkach, 2018–2025, http://sergetkach.com/
 */
				...
				if ($product['image']) {
					...
					if (isset($image_info['extension'])) {
						...
						if ($this->config->get('feed_branched_sitemap_off_check_image_file')) {
							$image_path = $image_info['dirname'] . '/' . $image_info['filename'] . '-' . $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_width') . 'x' . $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_height') . '.' . $image_info['extension'];

							$image = HTTPS_SERVER . 'image/cache/' . $image_path;

							if (!is_file(DIR_IMAGE . 'cache/' . $image_path)) {
								// Report :)
								$this->log->write('Notice - Branched Sitemap :: Image "' . $image . '" not exists on page ' . $this->url->link('product/product', '&product_id=' . $product['product_id']));
							}
						} else {
...
//			$output .= '<changefreq>monthly</changefreq>' . PHP_EOL;
	...
	public function friendlyURLWithoutHtaccess() {
		...
		if (false !== strpos($this->request->server['REQUEST_URI'], $branched_sitemap_url_base)) {
			...
			// Prevent opening page with $branched_sitemap_url_base without XML
			if ('' == $action) {
				if (false === strpos($uri, $this->config->get('feed_branched_sitemap_url'))) {
					return;
				}
			} else {
				// Білий екран + статус 200, якщо щось дописати навмання в УРЛ.
				// Або змінити адрес з branched-sitemap.xml на sitemap.xml, то стара адреса так само даcть білий екран
				// https://site.ua/sitemap.xml - OK
				// https://site.ua/blabla-sitemap - білий екран + код 200
				if (!in_array(ltrim($action, '/'), [
					'main',
					'categories',
					'products',
					'manufacturers',
			...
			exit; // To not shop standart 404 page after sitemap
		}
	}
