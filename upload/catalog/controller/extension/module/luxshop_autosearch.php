<?php
class ControllerExtensionModuleLuxshopAutosearch extends Controller {
	private $error = array();

	public function ajaxLiveSearch() {
		$json = array();
		if(!empty($this->request->get['filter_name'])){
			$this->load->model('extension/module/luxshop_autosearch');
			...
			$results = (array) $this->model_extension_module_luxshop_autosearch->ajaxLiveSearch($filterdata);
