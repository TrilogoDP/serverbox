<?php
class ControllerExtensionModuleLuxshopEditorplus extends Controller {
	public function index() {
		...
		if(isset($this->session->data['user_token'])){
			return $this->load->view('extension/module/luxshop_editorplus', $data);
		} else {
			return false;
		}
	}
	...
	if($this->request->post['route_mod'] == 'product/category'){
		...
		if(!empty($products_id)){
			$json['group_btn'] = $this->load->view('extension/module/luxshop_editorplus_group', $data);
		} else {
		...
		foreach ($products_id as $index=>$product_id) {
			...
			$astickers .= $this->load->view('extension/module/luxshop_editorplus_category', $data);
		...
		if($this->request->post['route_mod'] == 'product/product'){
			...
			if(isset($this->session->data['user_token'])){
				if (isset($this->request->post['product_id'])) {
					...
					$json['edit_prod'] = $this->load->view('extension/module/luxshop_editorplus_product', $data);
					header ('Content-type: text/html; charset=utf-8');
					echo json_encode($json);
			}
		}
