<?php
class ControllerExtensionModuleLuxshopWallcategory extends Controller {
	public function index($setting) {
		$data['type_view_mob'] = (!empty($setting['type_view_mob']) ? $setting['type_view_mob'] : 0);
		$this->document->addStyle('catalog/view/theme/luxshop/stylesheet/jquery.mCustomScrollbar.min.css');
		$this->document->addScript('catalog/view/theme/luxshop/js/jquery.mCustomScrollbar.min.js');
		$this->document->addScript('catalog/view/theme/luxshop/js/jquery.mousewheel-3.1.3.js');
		...
		$this->load->model('extension/module/luxshop_wallcategory');
		...
		if(!empty($category_info)){
			...
			if($subcategories){
				...
				foreach($subcategories as $subcategory){
					$path = $this->model_extension_module_luxshop_wallcategory->getCategoryPath($subcategory['category_id']);
					$data['subcategories'][] = array(
			...
			$path = $this->model_extension_module_luxshop_wallcategory->getCategoryPath($category['category']);
			$data['categories'][] = array(
				'width' 		=> $width,
				'height' 		=> $height,
		...
		return $this->load->view('extension/module/luxshop_wallcategory', $data);
	}
