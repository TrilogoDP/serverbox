<?php
class ControllerProductLuxshopLatestpage extends Controller {
	...
	public function index() {
		...
		if(isset($data['setting_lp']['status_latest_page']) && ($data['setting_lp']['status_latest_page'] == 1)){
			...
			$this->load->language('product/luxshop_latestpage');
			...
			$this->load->model('catalog/luxshop_latestpage');
			...
			if(isset($data['setting_lp']['status_receipt_date']) && ($data['setting_lp']['status_receipt_date'] == 1)){
				$date_availeble = $this->model_catalog_luxshop_latestpage->getDateAvaileble();
				if($date_availeble){
					foreach($date_availeble as $result_date){
						$data['date_availeble'][] = array(
							'text'  => $result_date['date_available'],
							'href'  => $this->url->link('product/luxshop_latestpage', 'date_ave=' . $result_date['date_available'] . $url)
						);
					}
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('product/luxshop_latestpage', $url)
			);
			$data['products'] = array();
			...
			$product_total = $this->model_catalog_luxshop_latestpage->getTotalProducts($filter_data);
			$results = $this->model_catalog_luxshop_latestpage->getLatest($filter_data);
			...
			$data['sorts'][] = array(
				...
				'href'  => $this->url->link('product/luxshop_latestpage', 'sort=p.date_added&order=DESC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'pd.name-ASC',
				'href'  => $this->url->link('product/luxshop_latestpage', 'sort=pd.name&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'pd.name-DESC',
				'href'  => $this->url->link('product/luxshop_latestpage', 'sort=pd.name&order=DESC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_price_asc'),
				'value' => 'p.price-ASC',
				'href'  => $this->url->link('product/luxshop_latestpage', 'sort=p.price&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_price_desc'),
				'value' => 'p.price-DESC',
				'href'  => $this->url->link('product/luxshop_latestpage', 'sort=p.price&order=DESC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_model_asc'),
				'value' => 'p.model-ASC',
				'href'  => $this->url->link('product/luxshop_latestpage', 'sort=p.model&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_model_desc'),
				'value' => 'p.model-DESC',
				'href'  => $this->url->link('product/luxshop_latestpage', 'sort=p.model&order=DESC' . $url)
			);
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('product/luxshop_latestpage', $url . '&limit=' . $value)
				);
			}
			...
			$pagination->url = $this->url->link('product/luxshop_latestpage', $url . '&page={page}');
			...
			if ($page == 1) {
				$this->document->addLink($this->url->link('product/luxshop_latestpage', '', true), 'canonical');
			} elseif ($page == 2) {
				$this->document->addLink($this->url->link('product/luxshop_latestpage', '', true), 'prev');
			} else {
				$this->document->addLink($this->url->link('product/luxshop_latestpage', 'page='. ($page - 1), true), 'prev');
			}

			if ($limit && ceil($product_total / $limit) > $page) {
				$this->document->addLink($this->url->link('product/luxshop_latestpage', 'page='. ($page + 1), true), 'next');
			}
			...
			$this->response->setOutput($this->load->view('product/luxshop_latestpage', $data));
		} else {
