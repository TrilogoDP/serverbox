<?php
class ModelExtensionModuleLuxshopProductcategorytabs extends Model {
	...
	public function getProductToCategoryTabSelect($data = array()) {
		...
		$sql = "SELECT * FROM (SELECT p.product_id, p.sort_order, p.image, pd.name, p.quantity, p.price,(SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
		...
		$sql .= " ORDER BY (p.price>0) DESC,(p.image>'') DESC,(p.quantity>0) DESC,LCASE(pd.name)" . $order;
		$sql .= " LIMIT  0," . (int)$data['limit_max'];
		$sql .= ") p ORDER BY " . $data['sort_order'] ." ,(p.price>0) DESC,(p.image>'') DESC,(p.quantity>0) DESC, LCASE(p.name)" . $order;
